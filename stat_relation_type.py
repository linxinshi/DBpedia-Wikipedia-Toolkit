...
def main():
    ...
    for line_src in lines:
        ...
        if count_relation.has_key(relation)==False:
           count_relation[relation]=0
           type_relation[relation]=set()
           
        count_relation[relation]+=1
        if len(type.strip())>0:
           type_relation[relation].add(type)
    ...
    list_relation=sorted(count_relation.iteritems(), key=lambda pair:pair[1], reverse = True)
    for rel,count in list_relation:
        if len(type_relation[rel])==0:
           type='STRING'
        else:
           type='|'.join(list(type_relation[rel]))
        print '%s\t%s\t%d'%(rel,type,count)
